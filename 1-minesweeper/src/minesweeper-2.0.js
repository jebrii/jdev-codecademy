...
const getNumberOfNeighborBombs = (bombBoard, rowIndex, colIndex) => {
  ...
  neighborOffsets.forEach(offset => {
    ...
    if (
      ...
      debugMsg(
        `Checking offset at index ${neighborRowIndex}, ${neighborColIndex}`
      );

      if (bombBoard[neighborRowIndex][neighborColIndex] === "B") {
        debugMsg(
          `-> I found a bomb at index ${neighborRowIndex}, ${neighborColIndex}`
        );
        adjacentBombs++;
      } else {
      ...
      debugMsg(
        `Tile at index ${neighborRowIndex}, ${neighborColIndex} is not valid.`
      );
    }
  });
...
const flipTile = (playerBoard, bombBoard, rowIndex, colIndex) => {
  if (playerBoard[rowIndex][colIndex] !== " ") {
    ...
    debugMsg(
      `The tile at ${rowIndex}, ${colIndex} has not been flipped. Checking for adjacent bombs.`
    );
